...
Python 3.12. numpy, scipy, networkx for the routing graph, rtree for spatial indexing, [osmium](https://osmcode.org/osmium-tool/) for parsing the OpenStreetMap (OSM PBF) extract, [shapely](https://shapely.readthedocs.io/) for geometry. The calibration measurements use 307 transitions from 40 held-out trips; the capacity measurements use 348 transitions from 50 trips.
